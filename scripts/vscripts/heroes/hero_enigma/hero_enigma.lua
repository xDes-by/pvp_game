...
LinkLuaModifier( "modifier_npc_dota_hero_enigma_permanent_ability_thinker", "heroes/hero_enigma/hero_enigma", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_npc_dota_hero_enigma_permanent_ability_helper", "heroes/hero_enigma/hero_enigma", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modifier_npc_dota_hero_enigma_permanent_ability_black_hole", "heroes/hero_enigma/hero_enigma", LUA_MODIFIER_MOTION_HORIZONTAL )

function npc_dota_hero_enigma_permanent_ability:OnHeroLevelUp()
	if IsServer() then
...
function modifier_npc_dota_hero_enigma_permanent_ability_helper:OnCreated( kv )
...
function modifier_npc_dota_hero_enigma_permanent_ability_helper:OnIntervalThink()
	if IsServer() and self:GetAbility() and self:GetCaster():IsRealHero() and self:GetCaster():IsAlive() and not self:GetParent():PassivesDisabled() then
		if self:GetAbility():IsCooldownReady() then
			...
			self.thinker = CreateModifierThinker(caster,self:GetAbility(),"modifier_npc_dota_hero_enigma_permanent_ability_thinker",{ duration = duration },point+RandomVector(RandomInt(1,700)),caster:GetTeamNumber(),false)
...
function modifier_npc_dota_hero_enigma_permanent_ability_black_hole:GetOverrideAnimation()
	return ACT_DOTA_FLAIL
end

function modifier_npc_dota_hero_enigma_permanent_ability_black_hole:GetOverrideAnimationRate()
	return 0.2
end
