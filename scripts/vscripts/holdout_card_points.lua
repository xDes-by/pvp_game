...
function GetPlayerAbilityNumber(hero)
    ...
    for i = 0, 30 do
        ...
        if ability ~= nil then 
            ...
            if AllSceptersAbility[abilityName] ~= true and add_spells[abilityName] ~= true and not string.match(abilityName, "special_bonus") and not string.match(abilityName, "ability_capture") and
    ...
    if PlayerResource:HasSelectedHero(nPlayerID) then
        ...
        if hero:HasAbility(abilityName) then
            ...
            if hero:IsChanneling() 
				...
				CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(nPlayerID), "CreateIngameErrorMessage", {message="#DOTA_HUD_Spells_Menu_Not_Sell"})
			...
			holdout_card_points:RemoveAbilityForEmpty(hero ,abilityName)

            local existingAbility = hero:FindAbilityByName(abilityName)
			hero:SetAbilityPoints(hero:GetAbilityPoints() + existingAbility:GetLevel())
            ...
            if additionalAbility then
                ...
                for _, associatedAbilityName in pairs(additionalAbility) do
					holdout_card_points:RemoveAbilityForEmpty(hero ,associatedAbilityName)
					hero:RemoveAbility(associatedAbilityName)
                end
            end
            if scepterAbility then
				print("remove scepter ability")
                for _, associatedLearnableName in pairs(scepterAbility) do
                    local learnableAbility = hero:FindAbilityByName(associatedLearnableName)
					holdout_card_points:RemoveAbilityForEmpty(hero ,associatedLearnableName)
                    hero:RemoveAbilityByHandle(learnableAbility)
                end
            end
            ...
            if hero:HasModifier("modifier_"..abilityName) or hero:HasModifier("modifier_"..abilityName.."_aura") then
                hero:RemoveModifierByName("modifier_"..abilityName)
                hero:RemoveModifierByName("modifier_"..abilityName.."_aura")
            end
            -- hero:SetAbilityPoints(abilityPointsToSet)
            ...
            if GetPlayerAbilityNumber(hero) >= maxSlotNumber then
				CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(nPlayerID), "CreateIngameErrorMessage", {message="#DOTA_HUD_Spells_Menu_Not_Enough_Slot"})
                return
            end
            ...
            while (hero:HasAbility("generic_hidden")) do
           ...
           -- newAbility:SetHidden(false)
			...
			-- local secondAbility = hero:FindAbilityByName(hero:GetName().."_permanent_ability")
			-- hero:SwapAbilities(abilityName, hero:GetName().."_permanent_ability", not newAbility:IsHidden(), not secondAbility:IsHidden())
   
            if hero:HasModifier("modifier_"..abilityName) or hero:HasModifier("modifier_"..abilityName.."_aura") then
                hero:RemoveModifierByName("modifier_"..abilityName)
                hero:RemoveModifierByName("modifier_"..abilityName.."_aura")
                hero:RemoveModifierByName("modifier_"..abilityName.."_passive")
            end
			...
			-- holdout_card_points:add_ability(abilityName, hero)
			...
			if additionalAbility then
                ...
                for _, associatedAbilityName in pairs(additionalAbility) do
                    local associatedAbility = hero:AddAbility(associatedAbilityName)
                    associatedAbility:SetLevel(1)
					-- holdout_card_points:add_ability(associatedAbilityName, hero)
                end
            end
			...
			print("add scepter ability")
                for _, associatedLearnableName in pairs(scepterAbility) do
                    local associatedLearnable = hero:AddAbility(associatedLearnableName)
					associatedLearnable:SetLevel(1)
					-- holdout_card_points:add_ability(associatedLearnableName, hero)
			...
			CustomGameEventManager:Send_ServerToPlayer(player, "dota_ability_changed", { entityIndex = hero })
        end
	end	
end


-- function holdout_card_points:add_ability(abilityName, hero)
	-- local maxSlotNumber = 5
	-- if hero:HasModifier("modifier_item_mulberry") then
		-- local stack_count = hero:FindModifierByName("modifier_item_mulberry"):GetStackCount()
		-- maxSlotNumber = maxSlotNumber + stack_count
	...
	-- for i=0, maxSlotNumber - 1 do
		...
		-- if not slot_ability.used_placeholder then
			-- newAbility = hero:AddAbility(abilityName)
			-- --hero:SwapAbilities(slot_ability:GetAbilityName(), newAbility:GetAbilityName(), false, true)
			-- hero:SwapAbilities(slot_ability:GetAbilityName(), newAbility:GetAbilityName(), true, true)
...
function holdout_card_points:RemoveAbilityForEmpty(hero ,ability_name)
	...
	if index <= 4 then
		hero:SwapAbilities(ability_name, "empty_" .. index, true, true)
...
function holdout_card_points:_GetPlayerSpells(nPlayerID)
    ...
    if player and PlayerResource:HasSelectedHero(nPlayerID) then
        ...
        local maxAbilities = hero:GetAbilityCount() - 1
...
function holdout_card_points:_SwapAbilitiesPosition(nPlayerID, firstAbilityName, secondAbilityName)
    local player = PlayerResource:GetPlayer(nPlayerID)
    local hero = player:GetAssignedHero()
    local firstAbility = hero:FindAbilityByName(firstAbilityName)
    local secondAbility = hero:FindAbilityByName(secondAbilityName)

    if firstAbility and secondAbility then
        hero:SwapAbilities(firstAbilityName, secondAbilityName, not firstAbility:IsHidden(), not secondAbility:IsHidden())
